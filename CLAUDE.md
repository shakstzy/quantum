...
| "render with remotion" / "compose this video" / "make a vertical 9:16 with captions" / "templated short-form video" / "build a Remotion project" / "generate clip from template" | Read `_core/skills/remotion/SKILL.md`. Programmatic video composition via Remotion 4.x (React-based). Drives the Remotion CLI to render mp4 from a typed composition + JSON props. Local-only, no auth. Default codec h264 with VideoToolbox on Apple Silicon. |
