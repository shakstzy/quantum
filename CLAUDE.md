...
| "post to IG" / "publish this reel" / "drop this on TikTok" / "upload to YouTube" / "cross-post this" | Read `~/SHAKOS/system/_core/playbooks/zernio-post/PLAYBOOK.md` (zernio-post skill). Cross-platform publish via Zernio direct REST. `PUBLISH` gate required. `ZERNIO_API_KEY` lives in `.claude/settings.local.json`. |
