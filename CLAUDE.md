...
| "ask grok X" / "grok this" / "use grok for X" / "grok think mode" / "grok deepsearch X" / "what does grok say about X" / "run X through grok" / "ask my grok" | Read `_core/skills/grok-web/SKILL.md`. Drives grok.com via patchright over Adithya's X Premium-linked Chrome session at `~/.quantum/chrome-profiles/grok/`. Verbs: `login` (one-time visible Sign in with X), `whoami`, `chat "<prompt>"` with `--model <name>` and `--mode default\|think\|deepsearch`, `quota`, `status`, `reset-breaker`, `diag`. Hybrid pattern: drives Tiptap composer + unified model/mode picker (`useModelModeSelector3`), captures the NDJSON stream from `POST /rest/app-chat/conversations/new` for clean response text + citations + images. Outputs to `~/.quantum/skill-output/grok-web/<run-id>/{response.md, metadata.json}` (NEVER `raw/` -- generative skill). 39 unit tests for the parser; 2 rounds cross-family Codex+Gemini bug-hunt closed. Live-tested 2026-04-30: default chat, --mode think, --model Fast, quota probe (138/140 default pool, 50/2h Think pool) all green. ToS-sensitive (xAI more aggressive than OpenAI on bot mitigation per Gemini); single-strike breaker on Cloudflare/Turnstile/captcha. Do NOT use for the xAI billed API (api.x.ai) -- that would be a separate `grok-cli` skill, not yet built. |
