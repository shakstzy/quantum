...
| "look up <address>" / "tell me about <address>" / "what do we know about <address>" / "all info on <address>" / "everything about this house: <address>" / "find homes in <city>" / "houses for sale in <zip>" / "details on this listing" / "Zestimate for X" / "Redfin Estimate for X" / "price history for <address>" / "comps for <address>" / "tax history for <address>" / "school ratings for <address>" / "homes in this polygon" / "houses inside this bounding box" / "compare <city A> vs <city B>" / "search across <city A> and <city B>" / "AI summary for <address>" / "walk score for <address>" / "permits for <address>" / "parcel info for <address>" / "open houses near <address>" / "rent estimate for <address>" / "what could I rent <address> for" / "rental comps for <address>" / paste a redfin.com or zillow.com URL with "look up" / "details" / "summarize" | Read `_core/skills/real-estate/SKILL.md`. Free CLI scraper for Redfin + Zillow via `curl_cffi` TLS impersonation (no Apify, no proxy). PRIMARY entrypoint for "tell me about this house": `re lookup "<address>"` resolves to both Redfin AND Zillow via brave-search and returns a merged view with `_conflicts` array when sources disagree. Also supports polygon + bbox + multi-region search, ~25 extra Redfin fields (AI summary, walk/transit/bike, permits, parcel, weather, commute), filter flags (status, sort, year-built, lot-size, HOA, pool, garage, new-construction). Cross-source dedupe via `scripts/dedupe.py`. Region resolution piggybacks on the brave-search skill OR `--region-id` manual override. Cap 20 req/task, throttle when looping. Dev iteration is fixture-only (`.dev-fixtures/` + `tests/`); see `raw/learnings/2026-04-28-cache-html-during-scraper-dev.md`. Personal use only. |
