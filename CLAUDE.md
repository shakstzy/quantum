...
│   ├── bumble/                  (per-person markdown <first>-bumble-<city>.md; v0.1 scaffold)
...
│   ├── bumble/                  (patchright bot, no api.bumble.com; v0.1 scaffold, selectors pending discovery)
...
| bumble | bumble.com via patchright (browser automation) | Swipes, matches, threads, replies + nudges + extends. v0.1 scaffold. Bumble's web `/app/encounters` style routes return 404 from origin; live auth surface is at `bumble.com/get-started` ("Quick sign in"). Selectors pending discovery via `scripts/discover-dom.mjs` (run once after manual login). Bumble-specifics: 24h match expiry triages outbound, women send opener (or set Opening Move), Date mode is enforced via mode-guard, tighter caps than Tinder (50/day swipes, 10/hr messages). Ban-aversion priority #1; same hard-rules class as Tinder. |
...
| clipping | Whop/Vyro/Discord clipper bounties + yt-dlp long-form sources | Bounty-compliance and distribution system for paid UGC clipping. SQLite control plane at `~/.quantum/clipping/clipping.db`, mlx-whisper transcripts cached forever, Claude moment-ranking, Remotion vertical 9:16 compose, mandatory pre-publish gate, zernio-post publish (default dry-run). North-star: `paid_views_per_approved_publish`. |
| ad-factory | Adithya's own ecom products + IG/TikTok hashtag scrapes (patchright) | Self-improving AI UGC ad pipeline. Per-product: scrape top 50 niche videos, Gemma multimodal-analyzes, Claude writes 3 script variants, Higgsfield Marketing Studio renders, ffmpeg cuts hero + 9x16 + 1x1, zernio-post ships, scrapes own metrics at 24h/7d/30d, learns. Persistent host library at `shared/hosts/<topic>-<handle>/` (one niche per host, real social handles). Outputs at `~/.quantum/ad-factory/edits/<slug>/`, NEVER `raw/`. PUBLISH gate. v0 scaffold: stages exist, scripts are stubs that exit non-zero. |
| linkedin | linkedin.com via patchright (URL navigation + innerText + structural-href signals; ported from stickerdaniel/linkedin-mcp-server reference). | Capability layer for LinkedIn. Verbs: get-profile, list-threads, get-conversation, list-invites (received/sent), send-dm (compose deeplink), send-connect (`/preload/custom-invite/?vanityName=` deeplink), accept-invite (inline Accept on incoming requests), withdraw-invite, search-people, pull (cron), status, diag, login. Per-person markdown at `raw/linkedin/<slug>-linkedin.md`. Caps: 10 connects/day, 50/wk; 5 cold DMs/day, 30 to existing connections; 100 profile fetches/day. Pending-invite ceiling: fail-closed if >400 outstanding. Auto-quarantine on hard ban-signal. Live-smoked 2026-04-30 against the real account: 10/10 verbs returned real artifacts. |
...
| `icm audit` / "run the icm audit" / "scan quantum for drift" | Read `_core/skills/icm-audit/SKILL.md`; run `python3 _core/skills/icm-audit/scripts/audit.py`. Auto-runs every 15 min via launchd `com.shakstzy.quantum-icm-audit`. Outputs at `~/.quantum/audit/latest/`. |
| "post to IG" / "publish this reel" / "drop this on TikTok" / "upload to YouTube" / "tweet this" / "cross-post this" / "post to my discord server" / "send announcement to discord" / "drop this in #ann on discord" / "publish to my discord" | Read `_core/skills/zernio-post/SKILL.md`. Cross-platform ORGANIC publish via Zernio direct REST (IG, YT, TikTok, Twitter/X, Discord-via-managed-bot). `PUBLISH` gate required. `ZERNIO_API_KEY` lives in `.claude/settings.local.json`. Do NOT use for ads  -  use `zernio-ads` instead. Do NOT use for personal Discord DMs or posting AS Adithya  -  that's the patchright-driven `_core/skills/discord/` (selfbot). If a Discord phrase is ambiguous between bot-posting and personal-posting, ASK the user which identity. |
...
| "swipe on bumble" / "run a bumble session" / "pull my bumble matches" / "draft replies for bumble" / "send my bumble queue" / "who's in my bumble queue" / "bumble status" / "is bumble halted" / "log into bumble" / "verify bumble selectors" / "diag bumble" | Read `workspaces/bumble/CLAUDE.md`. Patchright-driven Bumble bot mirroring the tinder workspace doctrine. Profile at `workspaces/bumble/.profile/`; per-person markdown at `raw/bumble/<slug>.md`. Hard caps: 50 swipes/day, 10 messages/hour, skip 1-2 days/wk, single-strike breaker on Cloudflare Turnstile / photo-verify / login-wall / mode-not-Date / account-restriction. Auth is manual phone-number only (no OAuth). Bumble web app surface is partly killed (`/app/encounters` returns 404); live `bumble.com/get-started` Quick-sign-in is the auth gate. v0.1 selectors pending discovery via `scripts/discover-dom.mjs`. Do NOT load the launchd plists until selectors wired and one end-to-end real send has passed. |
...
| "what's the state of <entity/area>" / "is the <thing> on" / "is anyone home" / "what's the temp inside" / "turn on/off <entity>" / "set <light> to <brightness/color>" / "dim the <light>" / "run <scene>" / "activate <scene>" / "trigger <automation>" / "what's playing on <media player>" / "pause/resume <media player>" / "list my automations / scenes / scripts" / "what cameras / sensors / switches do I have" / "is HA up" / "ping home assistant" / "ha status" | Read `_core/skills/home-assistant/SKILL.md`. Wraps the official `hass-cli` (home-assistant-ecosystem, 558★, MIT) with a Keychain-sourced shim at `_core/skills/home-assistant/scripts/ha`. HA instance is the Raspberry Pi at `http://homeassistant.local:8123`; long-lived access token in macOS Keychain `service=quantum-home-assistant` (accounts `url`, `token`). Deliberately CLI not MCP - HA's tool surface and entity-state dumps would pour raw payloads straight into context every call. Pre-filter shell-side (`ha state list light.` + `awk`/`grep`/`head`) before output reaches chat. Use for state reads, service calls (turn on/off, set brightness, run scenes), automation triggering. Do NOT use for editing HA YAML configs, dashboards/themes, add-ons, or HACS. |
...
| "list my linear issues" / "what's assigned to me in linear" / "search linear for X" / "create a linear issue" / "update <ID>" / "assign <ID>" / "comment on <ID>" / "what's the status of <ID>" / "show me the <project> project in linear" / "what cycle am I in" / "list linear teams" / "linctl <verb>" | Read `_core/skills/linear/SKILL.md`. Drives `linctl` (dorkitude/linctl, Go/Cobra, MIT) over Linear's API as Adithya. Token in macOS Keychain `service=quantum-linear`; working copy at `~/.linctl-auth.json` (mode 600). Footguns: 6-month default filter on `issue list`/`issue search`/`project list` (use `--newer-than all_time`); completed/canceled hidden by default (use `--include-completed`); always pass `--json` on reads; no `issue mine` verb (use `issue list --assignee me`); omit `--team` for cross-team. No PUBLISH gate; cross-account or destructive ops (delete project, bulk archive, agent mention) need explicit `CONFIRM`. |
| "dm <name> on discord" / "text <name> on discord" / "message <name> on discord" / "read my dms with <name>" / "search my discord for X" / "post in #<channel> on discord AS me" / "read #<channel> on discord" / "who is <name> on discord" | Read `_core/skills/discord/SKILL.md`. Send/read Discord as Adithya's PERSONAL account via patchright-driven Chrome session. Cookies in `~/.quantum/chrome-profiles/discord/`. ToS-sensitive (self-bot surface); burner account recommended. Do NOT use to post as the Zernio managed bot to a server channel  -  that's `_core/skills/zernio-post/` (`platform: "discord"`). When a Discord phrase is ambiguous, ASK whether they want their personal account (this skill) or the Zernio bot (zernio-post). |
| "read this X thread" / "summarize this tweet thread" / "pull replies on <tweet-url>" / paste an x.com or twitter.com tweet URL with "summarize" / "explain" / "show me <handle>'s recent tweets on X" / "what is <handle> tweeting" / "search X for <query>" / "what did I bookmark on X" / "show my X bookmarks" / "show my X analytics" / "how is my X account doing" / "X impressions / engagements / profile visits" / "who am I logged in as on X" | Read `_core/skills/x-read/SKILL.md`. Read-only X (Twitter) primitives via patchright + Adithya's logged-in Chrome session. Six verbs: `login`, `whoami`, `thread`, `profile`, `bookmarks`, `analytics`, `search`. Captures the X client's organic GraphQL responses (no replay HTTP). Cookies in `~/.quantum/chrome-profiles/x/`. Single-strike breaker on any auth challenge (Adithya's main is X Premium). Do NOT use to post / reply / like / follow / DM  -  posting goes through `_core/skills/zernio-post/` (`platform: "twitter"`, account `my-twitter`). For mass scrape across multiple burner accounts, sibling `x-scrape` skill is the future home (not yet built). |
...
| "gpt image of X" / "make me a gpt image" / "chatgpt image of X" / "openai image of X" / "generate an image with gpt" / "generate an image of X" (when not Higgsfield) | Read `_core/skills/gpt-images/SKILL.md`. Drives chatgpt.com via patchright over a persistent Chrome profile at `~/.quantum/chrome-profiles/chatgpt/`. Uses Adithya's existing ChatGPT plan, no OpenAI API key. Outputs to `~/.quantum/skill-output/gpt-images/<run-id>/` (NEVER `raw/`). One-time login required; cookies survive months. v1 is single-image only; multi-image responses pick the last. ToS-sensitive (browser automation against chatgpt.com). |
| "ask grok X" / "grok this" / "use grok for X" / "grok think mode" / "grok heavy X" / "deep research with grok" / "what does grok say about X" / "run X through grok" / "ask my grok" | Read `_core/skills/grok-web/SKILL.md`. Drives grok.com via patchright over Adithya's X Premium-linked Chrome session at `~/.quantum/chrome-profiles/grok/`. Verbs: `login` (one-time visible Sign in with X), `whoami`, `chat "<prompt>"` with `--model auto\|fast\|expert\|heavy\|grok-4.3` (legacy `--mode think` aliases to Expert; `--mode deepsearch` fails loud as removed-by-xAI), `quota`, `status`, `reset-breaker`, `diag`. **Model surface (verified 2026-05-02):** unified picker -- Auto / Fast / Expert / Heavy / Grok 4.3 (beta). Heavy + Grok 4.3 require SuperGrok Heavy subscription; the skill detects the upgrade dialog and exits 2 with a paywall message rather than hanging on intercepted clicks. Standalone DeepSearch mode was removed by xAI; redirect users to `--model heavy` (paywalled) or `--model expert` (free reasoning). Hybrid pattern: drives Tiptap composer + unified model picker (`useModelModeSelector3`), captures the NDJSON stream from `POST /rest/app-chat/conversations/new` for clean response text + citations + images. Outputs to `~/.quantum/skill-output/grok-web/<run-id>/{response.md, metadata.json}` (NEVER `raw/` -- generative skill). 39 unit tests for the parser; 2 cross-family bug-hunt rounds + a 2026-05-02 self-heal pass (paywall-modal detection, picker auto-recovery, dual-bucket quota probe -- grok-3 140/2h, grok-4 50/2h). Live-tested 2026-05-02: --model fast (PINEAPPLE), --mode think→Expert (MANGO), --mode deepsearch fail-loud, --model heavy paywall fail-loud, quota dual-probe all green. ToS-sensitive (xAI more aggressive than OpenAI on bot mitigation per Gemini); single-strike breaker on Cloudflare/Turnstile/captcha. Do NOT use for the xAI billed API (api.x.ai) -- that would be a separate `grok-cli` skill, not yet built. |
...
| "scrape this URL" / "read this page" / "extract the contents of <url>" / "what does <url> say" / "save this article" / "pull the markdown from <url>" / "ingest this link" | Read `_core/skills/firecrawl/SKILL.md`. Firecrawl `/v1/scrape` via REST CLI (no MCP). Key in `.claude/settings.local.json` env (`FIRECRAWL_API_KEY`). Output is ephemeral by default — only promotes to `raw/library/` on explicit save. Skip YouTube/Instagram URLs (use those skills instead). Full-site crawl is gated. |
| "research X" / "deep dive on X" / "find sources on X" / "build a reading list on X" / "what do experts say about X" / "give me the lay of the land on X" | Read `_core/skills/web-research/SKILL.md`. Hybrid orchestrator: Claude does query gen + ranking + synthesis; Codex runs ONE red-team pass for retrieval-gap discovery. Outputs to `~/.quantum/skill-output/web-research/<runId>/` (graphify-ignored). Promotion to `raw/library/` requires explicit save. |
| "find clipping bounties" / "discover clipping campaigns" / "is this whop bounty real" / "clip this podcast for money" / "render clip vertical with captions" / "publish this clip" / "reconcile clipping payouts" / "what's my clipping north-star" | Read `workspaces/clipping/CLAUDE.md`. Bounty-compliance + render + distribute pipeline. SQLite control plane is the spine; filesystem is artifact storage. Default publish is dry-run; live posting requires `LIVE=1` env + zernio-post `PUBLISH` token. Banned-niche regex in `workspaces/clipping/shared/policy/banned-niches.md` is non-negotiable (gambling, sportsbook, crypto trading, get-rich-quick, OnlyFans, conspiracy, manosphere). FTC `#ad` disclosure required for paid bounty posts. |
| "make a ugc ad for <product>" / "build an ad for this product" / "scaffold a new product in ad-factory" / "scaffold a new host" / "research what's winning in <niche>" / "render the picked script" / "ship the ad-factory edits" / "compute metrics on shipped ads" / "run the ad-factory learn pass" | Read `workspaces/ad-factory/CLAUDE.md`. 7-stage pipeline (research -> script -> render -> edit -> ship -> metrics -> learn). Operator triggers: `bash workspaces/ad-factory/scripts/cli.sh new <slug>` to scaffold a brief; `cli.sh host-new <topic> <handle>` to clone a host from `_template/`. Stage entrypoints at `stages/0N-<name>/scripts/run.sh`. Hosts are bound to one niche; mp4 scrapes are ephemeral; learnings persist. PUBLISH gate via `_core/skills/zernio-post/`. v0 scaffold: stage scripts are stubs that exit non-zero with "not yet wired"; implementation order is render -> edit -> script -> research -> ship -> metrics -> learn. |
| "render with remotion" / "compose this video" / "make a vertical 9:16 with captions" / "templated short-form video" / "build a Remotion project" / "generate clip from template" / "add animated captions to this audio" / "add scene transitions" / "render the same video for N inputs" | Read `_core/skills/remotion/SKILL.md`. Programmatic video composition via Remotion 4.x (React-based). Canonical install at `workspaces/clipping/remotion/` (Remotion 4.0.454). Add-ons installed: `@remotion/captions`, `@remotion/install-whisper-cpp` (local Whisper transcription), `@remotion/transitions`, `@remotion/google-fonts`. Drives the CLI to render mp4 from a typed composition + JSON props. Local-only, no auth. Default codec h264 with VideoToolbox on Apple Silicon. |
| "look up <address>" / "tell me about <address>" / "what do we know about <address>" / "all info on <address>" / "everything about this house: <address>" / "owner of <address>" / "who owns <address>" / "is <address> owner-occupied or investor-owned" / "find homes in <city>" / "houses for sale in <zip>" / "details on this listing" / "Zestimate for X" / "Redfin Estimate for X" / "price history for <address>" / "comps for <address>" / "tax history for <address>" / "school ratings for <address>" / "homes in this polygon" / "houses inside this bounding box" / "compare <city A> vs <city B>" / "search across <city A> and <city B>" / "AI summary for <address>" / "walk score for <address>" / "permits for <address>" / "parcel info for <address>" / "open houses near <address>" / "rent estimate for <address>" / "what could I rent <address> for" / "rental comps for <address>" / "appraised value for <address>" / "assessed value for <address>" / "deed date for <address>" / "homestead exemption for <address>" / "refresh travis cad" / "ingest the travis appraisal roll" / paste a redfin.com or zillow.com URL with "look up" / "details" / "summarize" | Read `_core/skills/real-estate/SKILL.md`. Three-layer free CLI: (1) Redfin + Zillow via `curl_cffi` TLS impersonation for active listings, history, schools, AI summary, walk/transit/bike, polygon/bbox/multi-region search, comps. (2) Rent estimation from Zillow rental comps. (3) **CAD (County Appraisal District) bulk-data layer** for owner names, county-appraised/assessed values, deed dates, exemption flags - free official appraisal-roll downloads ingested into local DuckDB; currently **Travis County (Austin)** with pluggable per-county adapters for the remaining 253 TX counties. PRIMARY entrypoint: `re lookup "<address>"` returns merged Redfin + Zillow + (when supported) CAD with `_conflicts` array when sources disagree. CAD-specific verbs: `re cad refresh travis` (one-time ingest, ~22s for 454k parcels), `re cad lookup "<addr>"`, `re cad summary`. Filter flags (status, sort, year-built, lot-size, HOA, pool, garage, new-construction). Cross-source dedupe via `scripts/dedupe.py`. Cap 20 req/task on the scrape side; CAD lookups are unlimited (local DB). Dev iteration is fixture-only (`.dev-fixtures/` + `tests/`); see `raw/learnings/2026-04-28-cache-html-during-scraper-dev.md`. Personal use only. |
| "add <book>" / "summarize this book" / "teach me <book>" / "I want to learn <book>" / "queue <book>" / "ask my library about X" / "what does the lit say about X" / "list my books" / "what's in my library" | Read `workspaces/library/CLAUDE.md`. LibGen scrape -> EPUB download -> pandoc -> `raw/library/books/<slug>/{source.epub, content.md, summary.md}`. Claude writes the summary body after the script lays the stub. Cross-book synthesis via Graphify (no NotebookLM). Scraper discipline: HTML cached to `.dev-fixtures/`, ≤5 live LibGen hits per dev session. |
...
- **Auto-run setup on workspace creation (Pattern 17).** Building a new workspace means: run `_core/templates/workspace/setup/questionnaire.md` interactively FIRST, collect answers, scaffold from template, substitute every `{{PLACEHOLDER}}`, verify zero `{{` remain, register in this Workspace Index, then run `python3 _core/skills/icm-audit/scripts/audit.py` to confirm clean. Never scaffold-then-ask.
