...
| "ask grok X" / "grok this" / "use grok for X" / "grok think mode" / "grok heavy X" / "deep research with grok" / "what does grok say about X" / "run X through grok" / "ask my grok" | Read `_core/skills/grok-web/SKILL.md`. Drives grok.com via patchright; profile at `~/.quantum/chrome-profiles/grok/`. Verbs: `login | whoami | chat "<prompt>" --model auto\|fast\|expert\|heavy\|grok-4.3 | quota | status | reset-breaker | diag`. Outputs to `~/.quantum/skill-output/grok-web/<run-id>/`. Heavy + Grok 4.3 are SuperGrok Heavy gated (fails loud with paywall msg). `--mode deepsearch` removed by xAI; use `--model heavy` (paywalled) or `--model expert`. Single-strike breaker on Cloudflare/Turnstile. NOT for the billed xAI API (api.x.ai)  -  separate `grok-cli` skill if ever built. |
