#!/usr/bin/env node
// match-delta v5c — full enumeration via mobile-UA, with touch-emulated +
// DOM-direct scrolling to try and unlock pagination beyond the v5b ceiling
// of 240 ids.
//
// Adds three scroll mechanisms (vs v5b's mouse.wheel only):
//   1) DOM scrollBy() on the actual matches scroll container (most reliable
//      for triggering IntersectionObserver-based pagination)
//   2) page.touchscreen.tap to keep "user is interacting" alive (cheap
//      heartbeat, won't open threads since it taps below content)
//   3) Mouse wheel (kept as baseline)
//
// Each iteration applies all three. Halt-safe before/after every scroll.
...
const RUN_OUT = "/tmp/tinder-match-delta-v5c.json";
...
const MIN_SCROLLS_BEFORE_STABLE = 25;
...
async function writeHalt(reason) {
  ...
  await writeFile(HALT_PATH, `match-delta-v5c: ${reason} at ${new Date().toISOString()}\n`);
  console.error(`!! HALT WRITTEN: ${reason}`);
}

// Find the matches list scroll container in the DOM and scroll it down
// natively. Returns {scrolled, scrollTop, scrollHeight, clientHeight, atBottom}.
async function scrollContainerNative(page, deltaPx) {
  return await page.evaluate((delta) => {
    // Walk up from a match anchor to find its closest scrollable ancestor
    const anchor = document.querySelector("a[href*='/app/messages/']");
    if (!anchor) return { scrolled: false, reason: "no anchor" };
    let el = anchor;
    while (el && el !== document.body) {
      ...
      if ((style.overflowY === "auto" || style.overflowY === "scroll" || style.overflowY === "overlay")
        ...
        return {
          ...
          atBottom: (after + el.clientHeight) >= (el.scrollHeight - 4),
        };
      }
      el = el.parentElement;
    }
    // Fallback: scroll window/document
    const docEl = document.scrollingElement || document.documentElement;
    const before = docEl.scrollTop;
    docEl.scrollBy({ top: delta, behavior: "auto" });
    const after = docEl.scrollTop;
    return {
      scrolled: after !== before,
      scrollTop: after,
      scrollHeight: docEl.scrollHeight,
      clientHeight: docEl.clientHeight,
      atBottom: (after + docEl.clientHeight) >= (docEl.scrollHeight - 4),
      fallback: "document",
    };
  }, deltaPx);
}
...
async function main() {
  await abortIfHalted();
  console.log("MATCH-DELTA v5c — full enumeration with multi-scroll-mechanism");
  console.log(`  scroll mix: mouse.wheel + DOM scrollBy on container + (optional touch tap)`);
  ...
  try {
    ...
    if (bannedFlag) { console.error("ban flag during initial load; aborting"); process.exit(3); }

    console.log(`\nafter initial load: ${callLog.length} calls, ${allIds.size} unique match_ids`);

    // Park mouse over the matches list area
    ...
    while (stable < STABLE_NEEDED && scrolls < MAX_SCROLLS && callLog.length < MAX_API_CALLS) {
      ...
      // Mechanism 1: humanScroll (mouse.wheel — same as v5b)
      await humanScroll(page, { distance: jitter(280, 540), steps: jitter(5, 9) });

      // Mechanism 2: DOM scrollBy on the actual scroll container
      const stat = await scrollContainerNative(page, jitter(380, 720));
      lastContainerStat = stat;
      ...
      const stateTag = stat.atBottom ? " [container@bottom]" : "";
      const counterPhase = scrolls < MIN_SCROLLS_BEFORE_STABLE
      ...
      if (newCalls === 0 && newIds === 0) {
        ...
        console.log(`scroll ${scrolls}: 0 new (${counterPhase}, total ${allIds.size})${stateTag}`);
      } else {
        stable = 0;
        console.log(`scroll ${scrolls}: +${newCalls} call(s), +${newIds} ids (total ${allIds.size})${stateTag}`);
      }
    ...
    console.log(`final container scroll state:  ${JSON.stringify(lastContainerStat)}`);
    ...
    const result = {
      ...
      last_container_stat: lastContainerStat,
      callLog,
      ids: [...allIds],
...
main().catch(e => { console.error(`match-delta v5c FAILED: ${e.stack || e.message}`); process.exit(1); });
