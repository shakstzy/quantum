// Cross-reference a Bumble match with iMessage activity.
// Reads macOS Contacts via osascript to find phone for a name, then scans
...
// First-name-only fuzzy match was catastrophic on the Tinder side: with thousands
// of contacts, multiple Sarahs/Annas/Emilys all match. Same risk applies here.
// Require BOTH first AND last name. If only first is known (typical), return null.
...
export function recommendChannel(activity, { silenceThresholdDays = 5 } = {}) {
  if (!activity) return "bumble_only";
  const { lastIn } = activity;
  if (!lastIn) return "bumble_only";
  const lastInDays = (Date.now() - new Date(lastIn).getTime()) / 86400000;
  if (lastInDays <= silenceThresholdDays) return "imessage_active";
  return "bumble_reengage";
}
